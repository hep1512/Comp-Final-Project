...
object OrderRepository {
    ...
    fun getOrders(username: String): List<OrderSummary> = transaction {
        ...
        DbOrders.selectAll()
            ...
            .map { row ->
                ...
                val date = row[DbOrders.placedAt]?.format(
    ...
    fun checkout(username: String, cart: List<CartLine>) = transaction {
    ...
    val existingAddress = Addresses.selectAll()
        .map { it }
        .firstOrNull { it[Addresses.userId].value == userId }

    val addressId = if (existingAddress != null) {
    ...
    val calculatedTotal = cart.sumOf { line ->
    ...
    // Insert order
    val orderId = DbOrders.insertAndGetId {
    ...
    fun totalOrdersCount(): Int = transaction {
        DbOrders.selectAll().count().toInt()
    }

    fun totalSalesAmount(): Double = transaction {
        DbOrders.selectAll()
