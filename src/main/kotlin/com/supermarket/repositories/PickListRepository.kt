...
object PickListRepository {
    ...
    fun activePickLists(): List<PickListOrder> =
        transaction {
            val usersById =
                Users.selectAll()
            ...
            val linesByOrder =
                (DbOrderItems innerJoin Products innerJoin Categories)
                    ...
                    .map { row ->
                        ...
                        row[DbOrderItems.orderId].value to
                            PickListLine(
            ...
            DbOrders.selectAll()
                .map { row ->
                    ...
                    PickListOrder(
                        ...
                        placedAt =
                            row[DbOrders.placedAt]?.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"))
                                ?: "Unknown",
    ...
    private fun statusSortOrder(status: String) =
        when (status) {
            "placed" -> 0
            "pending_payment" -> 1
            "picking" -> 2
